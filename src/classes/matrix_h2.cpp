...
matrix_h2::matrix_h2(arma::mat tX, field  (*kernel_fun)(const double *xx, const double *yy, void *data),
                                                              arma::mat hp, uint tleafsize,
...
////////////////////////////
arma::mat matrix_h2::solve_precond_vect(arma::vec tb,
                                           diagonal_precond *p, double eps,
                                           uint iter){
  
  pavector x = new_pointer_avector((double*)tb.memptr(),tb.n_rows);
  pavector b = new_avector(tb.n_rows);
  // solve_cg_h2matrix_avector(gp_h2, x, b,eps, 50);
  
  for(uint i = 0; i< b->dim; i++){
    b->v[i] = tb(i); //initial estimates of b set to be 0
  }

  uint ik = solve_pgmres_h2matrix_avector(gp_h2, diag_pre, p, x, b, eps, iter,200);
...
////////////////////////////
arma::mat matrix_h2::solve_precond_vect_wstart(arma::vec tb,
                                              diagonal_precond *p, double eps,
                                              uint iter,
                                              arma::mat start){
  ...
  for(uint i = 0; i< b->dim; i++){
    b->v[i] = start(i,0); //initial estimates of b set to be 0
  }
